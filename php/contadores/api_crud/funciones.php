...
function sumarTiempo($conexion, $id_socio, $usuario, $id_producto)
{
	...
	if ($resultado_producto->num_rows == 1) {
		...
		$duracion = $producto["duracion"]; // Asumiendo que la duración está en total

		// Verificar si ya existe un registro para el socio y la categoría
		$consulta_tiempo = "SELECT tiempo_total FROM tiempos_sala WHERE usuario=? AND categoria=?";
		...
		if ($resultado_tiempo->num_rows == 1) {
			...
			$consulta_actualizar = "UPDATE tiempos_sala SET tiempo_total=? WHERE usuario=? AND categoria=?";
